...
#define READ_16BIT_FROM_MEM(x) \
            (uint16_t) (machine->mem[x+1] << 8) + machine->mem[x]
...
int i8080_interrupt(i8080_t* machine, uint8_t restart) {
    ...
    machine->programCounter = (uint16_t) (restart << 3);
    return 11;
}
...
int i8080_execute(i8080_t* machine ) {
    ...
    const uint16_t memoryAddressRegister = i8080_get_hl(machine);
    ...
    switch (instruction) {
        ...
        case 0x33:
            // INX SP
            machine->stackPointer = machine->stackPointer + 1;
            cycles = 5;
            break;
        ...
        case 0xE9:
            // PCHL - jump to [HL]
            machine->programCounter = i8080_get_hl(machine);
            instructionLength = 0;
            cycles = 5;
            break;
        ...
        case 0xF5:
            ...
            tmp1 = (uint16_t)
                (machine->signFlag << 7 
                | machine->zeroFlag << 6 
                | machine->auxCarryFlag << 4 
                | machine->parityFlag << 2 
                | 1 << 1 
                | machine->carryFlag);
            
            machine->mem[machine->stackPointer - 2] = tmp1 & 0xff;
            machine->stackPointer -= 2;
            cycles = 11;
            break;
        ...
        case 0xF9:
            // SPHL - set SP to HL
            machine->stackPointer = i8080_get_hl(machine);
            cycles = 5;
            break;
